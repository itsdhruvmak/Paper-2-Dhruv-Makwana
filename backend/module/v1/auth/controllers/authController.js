...
export const userSignup = async (req, res) => {
    try {
        ...
        if (userData.loginType === "N") {
            ...
            if (existingUser) {
                return res.status(400).json({ message: "User already registered with this email", data: existingUser })
            }
        } else {
            const existinguser = await findSocialUser(userData.loginType, userData.socialId)
            if (existinguser) {
                return res.status(400).json({ message: "User already registered with this social id", data: existinguser })
            }
        }
        ...
        return res.status(201).json({ message: "User registered successfully", data: savedUser, refreshToken })
...
export const userLogin = async (req, res) => {
    try {
        ...
        if (loginType === "N") {
            ...
            if (!user) return res.status(400).json({ message: "User not found, you should register first" })
            ...
            if (!user) return res.status(400).json({ message: "Social id not found, you should first register" })
        }
        ...
        return res.status(200).json({ message: "User logged in successfully", data: user, refreshToken })
